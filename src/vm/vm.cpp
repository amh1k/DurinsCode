...
bool VM::executeAction(const std::string &actionName, GameState &state) const
{
    for (const auto &action : bytecode_["actions"])
        ...
        {
            executeInstructions(action["instructions"], state);
            return true;
        }
    return false;
}
...
void VM::executeInstructions(const json &instructions, GameState &state) const
{
    ...
    while (ip < instructions.size())
    {
        ...
        else if (op == "COMPARE_GT")
        {
            temps[dest] = state.intAttrs.count(src1) && state.intAttrs.at(src1) > ival;
        }
        else if (op == "COMPARE_GTE")
        {
            temps[dest] = state.intAttrs.count(src1) && state.intAttrs.at(src1) >= ival;
        }
        else if (op == "COMPARE_LT")
        {
            temps[dest] = state.intAttrs.count(src1) && state.intAttrs.at(src1) < ival;
        }
        else if (op == "COMPARE_LTE")
        {
            temps[dest] = state.intAttrs.count(src1) && state.intAttrs.at(src1) <= ival;
        }
        else if (op == "AND")
...
static void printHints(const json &bc, const GameState &state)
{
    ...
    for (const auto &action : bc["actions"])
    {
        ...
        if (!hasRoomCheck && hasItemCheck && !itemPresent)
            continue;
        if (roomOk)
            cmds.push_back(action["name"].get<std::string>());
    }
